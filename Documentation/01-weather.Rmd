...
If you don't know the FIPS code associated with the State you're querying for, use the following function to find it. This function will return a data table containing the FIPS code for each State. Alternatively, a Google Search will yield the same results.
...
#### Advanced Querying {-}
- **Fetching Daily Summaries for Multiple Variables** in a single query is not recommended. This is due to long processing times regardless of internet bandwidth/speed and large output data tables. However, you may choose to do this using the **apply** family of functions, as shown below.
