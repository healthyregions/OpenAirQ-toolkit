...
In this tutorial, we will take a close look at the land cover raster data of several large counties in the Midwest, and our goal is to map out the percentage of 1 km grid cells that are covered by low, mid, and high intensity urban land cover. At this point, it may not be clear what it is exactly that we intend to do, but everything will start to make sense once we inspect the available data. Let's load the packages and data first. 

## Loading Packages and Data

Among the packages we will be using, "tidyverse" is often used in all kinds of data manipulation and statistical analysis. The other three packages are more specialized and are mostly used for spatial analysis. It should be mentioned here that the "velox" package, which is great at fast raster data manipulation, is sadly no longer available. Instructions for installing this package from R archive are included in other parts of this tutorial book and can also be found [here](https://support.rstudio.com/hc/en-us/articles/219949047-Installing-older-versions-of-packages).
