...
In this tutorial, we will learn to deal with raster data. For illustrative purposes, we will take a look at the NDVI data (Normalized Difference Vegetation Index) of 21 large counties in the Midwest. 

# Loading Packages and Data

First, let's load the packages needed for this exercise. 
...
```{r extracting ndvi data}
#this.qtr <- ndvi.quarterly[[19]]
this.qtr <- subset(ndvi.quarterly, 19, drop = FALSE)
```
...
This plot gives an overview of the NDVI of the 21 lareg counties. Without the county borders explicitly drawn, we are unable to compare the NDVI across counties. Moreover, the plot is far from aesthetically pleasing.
...
```{r interavtive ndvi, message = FALSE, warning = FALSE}
tmap_mode("view")
...
```{r Plot Mode ndvi, message = FALSE, warning = FALSE}
tmap_mode("plot")
```
...
```{r summary ndvi, message = FALSE, warning = FALSE}
...
```{r Extract Cook County Shape ndvi}
...
```{r Plot Cook County Shape ndvi, fig.align='center'}
...
```{r Plot Cook County ndvi, fig.align='center'}
tm_shape(cook.ndvi) +
